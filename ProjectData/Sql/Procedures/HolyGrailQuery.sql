...
WHERE  (ST.StateAbbrev = @STATE OR @STATE IS NULL)
AND    (S.StoreAddress = @ADDRESS OR @ADDRESS IS NULL)
AND    (S.ZipCode = @ZIPCODE OR @ZIPCODE IS NULL)
AND    (S.StoreName = @STOREFRONT OR @STOREFRONT IS NULL)
AND	   (CC.ComponentCategoryName = @COMPONENT_CATEGORY OR @COMPONENT_CATEGORY IS NULL)
AND    (C.ComponentName = @COMPONENT_NAME OR @COMPONENT_NAME IS NULL)
AND    (M.ManufacturerName = @COMPONENT_MANUFACTURER OR @COMPONENT_MANUFACTURER IS NULL)
